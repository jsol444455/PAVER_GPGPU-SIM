/**new locality guru name based on the revisions: syr2k_4_16_32_8.ptx
 ...
 *
 * REVISED: Fixed bugs for N != M case
 */
...
/* Problem size - REVISED for faster simulation (~2 min) */
#define N 128
#define M 256
...
void init_arrays(DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *C)
{
	...
	for (i = 0; i < N; i++)
		...
		for (j = 0; j < M; j++)
		{
			/* FIX: Changed i*N to i*M for correct stride */
	  		A[i*M + j] = ((DATA_TYPE) i*j) / N;
	  		B[i*M + j] = ((DATA_TYPE) i*j + 1) / N;
		}
    	}
}
...
int main()
{
	...
	/* FIX: Changed N*M to N*N for correct C matrix size */
	C = (DATA_TYPE*)malloc(N*N*sizeof(DATA_TYPE));
	C_outputFromGpu = (DATA_TYPE*)malloc(N*N*sizeof(DATA_TYPE));
